...
train_data_1 = np.loadtxt('train1.txt')
train_data_2 = np.loadtxt('train2.txt')
test_data_1 = np.loadtxt('test1.txt')
test_data_2 = np.loadtxt('test2.txt')

x_train_1, y_train_1 = train_data_1[:, 0], train_data_1[:, 1]
x_train_2, y_train_2 = train_data_2[:, 0], train_data_2[:, 1]
x_test_1, y_test_1 = test_data_1[:, 0], test_data_1[:, 1]
x_test_2, y_test_2 = test_data_2[:, 0], test_data_2[:, 1]
...
history = ann.train_model(x_train_1, y_train_1, num_epochs=20000)

# PLOT LOSS VALUES
plt.scatter(range(len(history['loss'])), history['loss'], s=3, edgecolors='blue')
# plt.legend()
plt.grid()
plt.title('Loss Values (over train1 dataset)'.format())
...
# TRAIN OVER FIRST DATASET (TRAIN & TEST)
ann2.compile(learning_rate=0.01, activation='sigmoid')
history2 = ann2.train_model(x_train_2, y_train_2, num_epochs=200000)

# PLOT LOSS VALUES
plt.scatter(range(len(history2['loss'])), history2['loss'], s=2, edgecolors='blue')
# plt.legend()
plt.grid()
# plt.title('Loss values of models with momentum vs without momentum'.title())
plt.title('Loss Values (over train2 dataset)'.format())
...
# Test the hidden values in a sample prediction
print(ann.predict([3]))

def plot_hidden_units(x, model, dataset_number):
    a = np.min(x)
    b = np.max(x)
    interval = np.linspace(a, b, 100).reshape(-1, 1)  # Reshape for adaption to the model
    _, hidden_values = model.predict(interval)

    for n in hidden_values.keys():
        plt.plot(interval, hidden_values[n], label='n={}'.format(n))

    plt.legend(bbox_to_anchor=(1, 0), loc='lower right')
    plt.title('Hidden Values of Train_{}'.format(dataset_number))
    plt.savefig('hidden_unit_values_{}.png'.format(dataset_number))
    plt.show()


plot_hidden_units(x_train_1, ann, 1)
plot_hidden_units(x_train_2, ann2, 2)
